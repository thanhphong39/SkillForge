...
import { mockStrategySets } from '../data/mockStrategicObjectives.js'

export const useStrategyStore = create((set, get) => ({
  strategySets: mockStrategySets.map(s => ({
    ...s,
    objectives: s.objectives.map(o => ({ ...o })),
    links: s.links.map(l => ({ ...l })),
  })),
  activeStrategyId: mockStrategySets[0].id,
  selectedObjectiveId: null,

  getActiveStrategy: () => {
    const { strategySets, activeStrategyId } = get()
    return strategySets.find(s => s.id === activeStrategyId) ?? strategySets[0]
  },
  ...
  addStrategy: (name, description, color) => set(s => {
  ...
  deleteStrategy: (id) => set(s => {
    ...
    return {
      ...
      activeStrategyId: remaining.length > 0
        ? (s.activeStrategyId === id ? remaining[0].id : s.activeStrategyId)
        : null,
      selectedObjectiveId: null,
    }
  }),
